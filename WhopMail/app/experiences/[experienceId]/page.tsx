...
export default async function ExperiencePage({ params, searchParams }: ExperiencePageProps) {
  try {
    console.log("🔐 AUTH: === EXPERIENCE PAGE AUTHENTICATION START ===")
    ...
    // Check if user has access to this experience using proper Whop SDK authentication
    console.log("🔐 AUTH: === EXPERIENCE ACCESS VALIDATION ===")
    let accessResult
    if (isDevelopmentMode) {
      // In development mode, grant admin access
      console.log("🔐 AUTH: === DEVELOPMENT MODE ACCESS GRANT ===")
      console.log("🔐 AUTH: Granting admin access to user " + userId + " for experience " + resolvedParams.experienceId)
      accessResult = {
        hasAccess: true,
        accessLevel: 'admin'
      }
      console.log("🔐 AUTH: Access result:", JSON.stringify(accessResult, null, 2))
    } else {
      // Production mode - validate access using Whop SDK (proper authentication flow)
      console.log("🔐 AUTH: === PRODUCTION MODE ACCESS VALIDATION ===")
      try {
        console.log("🔐 AUTH: Calling whopSdk.access.checkIfUserHasAccessToExperience...")
        console.log("🔐 AUTH: Parameters: userId=" + userId + ", experienceId=" + resolvedParams.experienceId)
        ...
        console.log("🔐 AUTH: ✅ Access validation successful:")
        console.log("🔐 AUTH: User " + userId + " has " + accessResult.accessLevel + " access to experience " + resolvedParams.experienceId)
        console.log("🔐 AUTH: Full access result:", JSON.stringify(accessResult, null, 2))
      } catch (error) {
        console.error("🔐 AUTH: ❌ Error checking experience access:", error)
        console.error("🔐 AUTH: Error details:", JSON.stringify(error, null, 2))
        
        // In development mode, grant access despite API errors
        if (isDevelopmentMode) {
    ...
    // Admin-only app - only allow admin access
      ...
      return (
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              This app is only available to administrators.
            </p>
          </div>
        </div>
      )
