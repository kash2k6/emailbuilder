...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const response = await fetch(`https://api.whop.com/api/v2/products/${productId}`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Failed to fetch product from Whop" }, { status: response.status })
    }

    const data = await response.json()
    return NextResponse.json({ data })
  } catch (error) {
    console.error("Error fetching product from Whop:", error)
