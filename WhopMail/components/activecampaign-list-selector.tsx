...
export function ActiveCampaignListSelector({
  ...
  const saveIntegration = async (listId: string) => {
    try {
      ...
      console.log("ActiveCampaign integration saved to Supabase")
    } catch (error) {
      console.error("Failed to save ActiveCampaign integration:", error)
    }
  }
              ...
              <div key={list.id} className="flex items-center space-x-2">
                ...
                <Label htmlFor={`list-${list.id}`} className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {list.subscriber_count || 0} subscribers • Created {new Date(list.cdate).toLocaleDateString()}
                  </span>
                </Label>
              </div>
            ))}
